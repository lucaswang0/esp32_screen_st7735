...
基于 ESP32-C3 和 ST7735 1.44" 128×128 TFT 显示屏的温湿度时钟项目，带翻页时钟动画、Web 配置界面、图表页面和 MQTT 远程发布功能。
...
### 传感器
- **DHT11 ×1**：GPIO6
- **历史数据存储**：按日期保存到 SPIFFS `/sensor1_YYYY-MM-DD.dat`
...
└── main.cpp           # 主程序入口（Setup / Loop 页面调度）
```
...
| `taskDisplay` | 主循环 | 页面切换、翻页动画、传感器/图表重绘 |
